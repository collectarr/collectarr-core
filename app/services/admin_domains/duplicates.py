...
class AdminDuplicateService:
    ...
    async def merge_duplicate_candidate(
        ...
        *,
        ...
        conflicts = await self._duplicate_conflict_flags(all_ids)
        confidence_factors = self._duplicate_confidence_factors(
            [target, *sources], conflicts=conflicts
        )
        merge_warnings = self._duplicate_merge_warnings(conflicts)
        duplicate_score, recommended_target_id = self._score_duplicate_candidate(
            [target, *sources], conflicts=conflicts
    ...
    def _duplicate_merge_warnings(self, conflicts: dict[str, bool]) -> list[str]:
        ...
        return warnings
    def _merge_target_score(self, entity: Any) -> tuple[int, int, int]:
        ...
        if self._entity_primary_publisher(entity) is not None:
            score += 6
        score += child_count * 3
        return score, int(self._entity_has_cover(entity)), child_count
    ...
    async def _move_native_children(self, source: Any, target: Any) -> None:
        ...
        elif isinstance(source, MusicAlbum):
            await self._move_album_children(MusicAlbumDiscTitle, sid, tid, ("disc_number",))
            await self._move_album_children(
                MusicAlbumTrack, sid, tid, ("disc_number", "position")
            )
            await self._move_album_children(MusicAlbumCredit, sid, tid, ("role", "sequence"))

    async def _move_album_children(
        self,
        model: type[Any],
        source_id: UUID,
        target_id: UUID,
        ...
        children = await self.db.scalars(select(model).where(model.album_id == source_id))
        for child in children:
            identity = {field: getattr(child, field) for field in identity_fields}
            existing = await self.db.scalar(
                select(model.album_id).where(
                    model.album_id == target_id,
