...
class TVSeries(UuidMixin, TimestampMixin, Base):
    ...
    first_air_date_parts: Mapped[str | None] = mapped_column(String(64))
    last_air_date: Mapped[date | None] = mapped_column(Date)
    last_air_date_parts: Mapped[str | None] = mapped_column(String(64))
...
class MovieWork(UuidMixin, TimestampMixin, Base):
    ...
    entity_links: Mapped[list["EntityLink"]] = relationship(
        primaryjoin=lambda: and_(
            foreign(EntityLink.entity_id) == MovieWork.id,
            EntityLink.entity_type == "movie_work",
        ),
...
class MovieRelease(UuidMixin, TimestampMixin, Base):
    ...
    entity_links: Mapped[list["EntityLink"]] = relationship(
        primaryjoin=lambda: and_(
            foreign(EntityLink.entity_id) == MovieRelease.id,
            EntityLink.entity_type == "movie_release",
        ),
        order_by="EntityLink.position",
        viewonly=True,
    )
