...
def upgrade() -> None:
    ...
    item_kind = postgresql.ENUM(
        "anime", "boardgame", "book", "bluray", "comic",
        "game", "manga", "movie", "music", "tv",
        name="item_kind",
    )
    ...
    series_relation_type = postgresql.ENUM(
        ...
        "adaptation", "alternative", "summary", "compilation", "other",
    ...
    # re-bind with create_type=False for column references
    item_kind = postgresql.ENUM("item_kind", name="item_kind", create_type=False)
    external_provider = postgresql.ENUM("external_provider", name="external_provider", create_type=False)
    series_relation_type = postgresql.ENUM("series_relation_type", name="series_relation_type", create_type=False)
    ...
    op.create_table(
        ...
        sa.Column("sort_key", sa.String(255), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("cover_image_key", sa.String(512), nullable=True),
        sa.Column("cover_image_url", sa.String(1024), nullable=True),
        sa.Column("thumbnail_image_key", sa.String(512), nullable=True),
        sa.Column("thumbnail_image_url", sa.String(1024), nullable=True),
        sa.Column("description", sa.Text(), nullable=True),
        sa.Column("metadata_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "entity_type", "entity_id", "organization_id", "role",
            name="uq_entity_organization_role",
        ),
    )
    op.create_index("ix_entity_organizations_entity", "entity_organizations", ["entity_type", "entity_id"])
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "entity_type", "entity_id", "person_id", "role",
    ...
    op.create_table(
        ...
        sa.Column("entity_type", sa.String(64), nullable=False),
        sa.Column("entity_id", sa.UUID(), nullable=False),
        sa.Column("image_type", sa.String(64), nullable=False),
    ...
    op.create_index("ix_metadata_proposals_status_provider", "metadata_proposals", ["status", "provider"])
    op.create_index(op.f("ix_metadata_proposals_provider"), "metadata_proposals", ["provider"])
    op.create_index(op.f("ix_metadata_proposals_provider_item_id"), "metadata_proposals", ["provider_item_id"])
    op.create_index(op.f("ix_metadata_proposals_status"), "metadata_proposals", ["status"])
    ...
    op.create_index("ix_image_cache_last_accessed", "image_cache_entries", ["last_accessed_at"])
    op.create_index("ix_image_cache_provider_source", "image_cache_entries", ["provider", "source_url"])
    op.create_index(op.f("ix_image_cache_entries_content_hash"), "image_cache_entries", ["content_hash"])
    op.create_index(op.f("ix_image_cache_entries_provider"), "image_cache_entries", ["provider"])
    op.create_index(op.f("ix_image_cache_entries_provider_item_id"), "image_cache_entries", ["provider_item_id"])
    ...
    op.create_index("ix_provider_ingest_jobs_provider_item", "provider_ingest_jobs", ["provider", "provider_item_id"])
    op.create_index("ix_provider_ingest_jobs_status_next_run", "provider_ingest_jobs", ["status", "next_run_at"])
    op.create_index(op.f("ix_provider_ingest_jobs_item_id"), "provider_ingest_jobs", ["item_id"])
    op.create_index(op.f("ix_provider_ingest_jobs_next_run_at"), "provider_ingest_jobs", ["next_run_at"])
    op.create_index(op.f("ix_provider_ingest_jobs_provider"), "provider_ingest_jobs", ["provider"])
    op.create_index(op.f("ix_provider_ingest_jobs_provider_item_id"), "provider_ingest_jobs", ["provider_item_id"])
    op.create_index(op.f("ix_provider_ingest_jobs_status"), "provider_ingest_jobs", ["status"])
    ...
    op.create_index(op.f("ix_admin_audit_logs_actor_email"), "admin_audit_logs", ["actor_email"])
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
        sa.ForeignKeyConstraint(["source_series_id"], ["series.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["target_series_id"], ["series.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "source_series_id", "target_series_id", "relation_type",
            name="uq_series_relations_source_target_type",
        ),
    )
    op.create_index("ix_series_relations_source", "series_relations", ["source_series_id"])
    op.create_index("ix_series_relations_target", "series_relations", ["target_series_id"])
    op.create_index("ix_series_relations_type", "series_relations", ["relation_type"])
...
def downgrade() -> None:
    ...
    sa.Enum(name="series_relation_type").drop(op.get_bind(), checkfirst=True)
    sa.Enum(name="external_provider").drop(op.get_bind(), checkfirst=True)
