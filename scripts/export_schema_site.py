...
def serialize_column(
    ...
    foreign_keys = [
        serialize_foreign_key(column.name, foreign_key)
...
def serialize_constraints(table: Table) -> dict[str, Any]:
    ...
    uniques = [
        ...
        for constraint in sorted(
            ...
            key=lambda current: ([column.name for column in current.columns], current.name or ""),
        )
    ]
    checks = [
        {
        ...
        for constraint in sorted(
            ...
            key=lambda current: (str(current.sqltext), current.name or ""),
        )
    ]
    foreign_keys = [
        {
        ...
        for constraint in sorted(
            ...
            key=lambda current: (
                [element.parent.name for element in current.elements],
                [f"{element.column.table.name}.{element.column.name}" for element in current.elements],
                current.name or "",
            ),
        )
    ]
